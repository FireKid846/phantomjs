...
async function addWarning(memberId, reason) {
  await db.execute({
...
async function getWarnings(memberId) {
  const res = await db.execute({
...
async function clearWarnings(memberId) {
  await db.execute({ sql: `DELETE FROM warnings WHERE member_id = ?`, args: [memberId] });
  await db.execute({ sql: `UPDATE members SET warning_count = 0 WHERE id = ?`, args: [memberId] });
}
